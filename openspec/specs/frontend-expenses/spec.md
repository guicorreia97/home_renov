# frontend-expenses Specification

## Purpose

The expenses screen: how a single local user records renovation spend, sets the
budget that spend is measured against, and sees where they stand. This covers the
client's observable behaviour only — every total is computed server-side against
exact decimals and consumed here, never recalculated in the browser.
