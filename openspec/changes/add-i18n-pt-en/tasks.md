...
- [ ] 1.4 Implement `LocaleProvider` resolving the initial locale synchronously
...
- [ ] 2.1 Change `formatMoney(amount, currency, locale)` and
      ...
      guard still holds
- [ ] 2.2 Add `formatPercent(value, locale)` using `Intl.NumberFormat` with
      `style: 'percent'` on `value / 100`; verify a unit test asserts a comma
      separator under `pt-PT` and a point under `en`
- [ ] 2.3 Add a `useFormat()` hook binding the active locale into all three; verify
      a test shows a component using it re-formats when the locale changes
- [ ] 2.4 Replace `percent.toFixed(1)` in `RemainingBudgetConclusion.tsx` with
...
- [ ] 3.1 Add exhaustive label keys for `ExpenseCategory`, `PaymentMethod` and
...
- [ ] 3.2 Rewrite `enumOptions.ts` to build options from catalogue lookups and
...
- [ ] 3.3 Confirm the machine value still crosses the wire: verify a test in
...
- [ ] 4.1 `ExpensesScreen.tsx` and `App.tsx` (~8 strings, including the shell and
      backend-connection copy); verify no user-visible literal remains in either
      file and both render under `en`
- [ ] 4.2 `BudgetSummaryStrip.tsx` (~13 strings, including the "no budget set"
...
- [ ] 4.8 Replace any raw backend `detail` shown in the UI with localised failure
...
- [ ] 5.1 Build the language switcher in the app shell from the existing
...
- [ ] 5.2 Mount `LocaleProvider` in `main.tsx` above the app; verify the app
      boots in the detected language with no English flash before first paint
- [ ] 5.3 Set `document.documentElement.lang` from a provider effect and remove
      the hardcoded `lang="en"` assumption in `frontend/index.html`; verify a
      test asserts the attribute becomes `pt-PT` after switching
- [ ] 5.4 Verify switching mid-edit preserves state: a test fills the expense
      form, triggers a validation error, switches language, and asserts the
      values and the open modal survive, the error is re-rendered translated, and
      no fetch was made

## 6. The Portuguese catalogue

- [ ] 6.1 Fill every `pt-PT` value — European Portuguese orthography, not
...
- [ ] 7.1 Add `renderWithLocale(ui, locale)` to `frontend/src/test/`; verify it
      renders a consumer at a given locale without a real provider setup at each
      call site
- [ ] 7.2 Migrate the 146 text-based queries across the 13 test files, preferring
...
- [ ] 7.3 Run `ExpensesScreen`, `ExpenseFormModal` and `BudgetSettingsModal`
      tests under both locales; verify each asserts its language-specific
      formatting — money, date, percentage, enum label
- [ ] 7.4 Verify rewording does not break the suite: change one `en` value, run
      `npm test`, confirm green, revert the change

## 8. Layout under Portuguese

- [ ] 8.1 Open both modals in `pt-PT` with the longest labels present and check
      ...
      token
- [ ] 8.2 Render the summary strip in `pt-PT`; verify labels do not collide with
      their figures and the figures stay tabular-aligned

## 9. Docs

- [ ] 9.1 Add a text-expansion note to `docs/design-system-guide.md` — Portuguese
      runs 20–30% longer, the modal is fixed at 480px, and 13px is the floor;
      verify `make harness-check` passes
- [ ] 9.2 Document the catalogue convention and `renderWithLocale` in
...
- [ ] 10.1 Run `make check` and verify it passes in full — harness coherence,
      ruff, secret scan, pytest and the frontend gate
- [ ] 10.2 Spawn the `reviewer` sub-agent on the branch diff, paying particular
